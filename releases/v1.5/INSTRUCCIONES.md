# Sistema de Timbres Tora Or - v1.5
...
- ✅ **Eliminación de switches por timbre** - Se simplificó la gestión eliminando switches individuales por timbre

### Mejoras en MQTT / Home Assistant

- ✅ **Switches por día en HA** - Nuevos switches para controlar cada día de la semana desde Home Assistant
- ✅ **Switch maestro en HA** - Control global de todos los horarios desde Home Assistant
- ✅ **Iconos actualizados** - Iconos más intuitivos para cada tipo de control
- ✅ **Sincronización mejorada** - Mejor sincronización bidireccional entre HA y la interfaz web

### Feedback visual mejorado

- ✅ **Estados claros** - "Activo", "Inactivo" o "Día desactivado" con colores distintivos
- ✅ **Horarios agrupados** - Cuando varios timbres suenan a la misma hora, se muestran juntos
- ✅ **Vista por días** - Cada día de la semana tiene su propia sección organizada
...
- `tora-or-combined-v1.5.bin` - **NUEVO**: Archivo combinado con todo (firmware + filesystem + bootloader + particiones) - La forma más fácil de flashear
- `tora-or-firmware-v1.5.bin` - Firmware principal del sistema
- `tora-or-filesystem-v1.5.bin` - Sistema de archivos con páginas web
...
4. Selecciona `tora-or-firmware-v1.5.bin`
...
7. **Para actualizar el filesystem** (recomendado):
   ...
   - Esta vez selecciona `tora-or-filesystem-v1.5.bin`
...
6. Sube los archivos en este orden:
   ...
   - `0x10000`: `tora-or-firmware-v1.5.bin`
   - `0x290000`: `tora-or-filesystem-v1.5.bin`
...
```bash
python -m esptool --chip esp32 --port COM4 --baud 921600 write_flash -z 0x0 tora-or-combined-v1.5.bin
```
...
```bash
python -m esptool --chip esp32 --port COM4 --baud 921600 write_flash -z --flash_mode dio --flash_freq 40m --flash_size detect 0x1000 bootloader.bin 0x8000 partitions.bin 0x10000 tora-or-firmware-v1.5.bin 0x290000 tora-or-filesystem-v1.5.bin
```
...
```bash
python -m esptool --chip esp32 --port COM4 --baud 921600 write_flash -z 0x10000 tora-or-firmware-v1.5.bin 0x290000 tora-or-filesystem-v1.5.bin
```
...
### Desde v1.4, v1.3 o anterior:

1. **Crear backup**: Ve a Configuración → Backup y Restauración → Descargar Backup
2. **Instalar v1.5**: Usa el método OTA o cualquiera de los métodos de instalación arriba
...
### ⚠️ Cambios importantes después de actualizar
